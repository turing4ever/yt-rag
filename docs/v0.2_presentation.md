...
| Metric | Value | Notes |
|--------|-------|-------|
| **Total Tests** | 163 | Across 8 YouTube channels |
| **Query Classification Accuracy** | 59.5% | Room for improvement |
| **Keyword Match Rate** | 83.0% | Retrieval works well |
| **LLM Validation Pass Rate** | 31.9% | Answer generation needs work |

### Timing Breakdown (avg per query)
| Stage | Time | % of Total |
|-------|------|------------|
| Parallel (embed + analyze) | 477ms | 19% |
| FAISS Search | 17ms | <1% |
| Keyword Filtering | 14ms | <1% |
| Answer Generation | 1,347ms | 54% |
| Validation | 638ms | 26% |
| **Total** | **2,496ms** | 100% |

**Insight:** Answer generation dominates latency. FAISS is blazing fast.
...
### The 83% vs 32% Gap
- **83%** keyword match = retrieval finds relevant content
- **32%** LLM validation = answer generation needs work
- Retrieval > Generation in current system
...
6. **Fix retrieval first** - 83% retrieval vs 32% generation success
